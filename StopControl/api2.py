...
# Ensure keys added after the original release exist in older db.json files
if "obstacle" not in data:
    data["obstacle"] = {"id": 18, "position": [0.0, 0.0, 0.0], "found": False}
if "agentStop" not in data:
    data["agentStop"] = [{"id": i+1, "stop": 0} for i in range(NUM_ROBOTS)]
...
# agentStop route — written by checkSafety, read by checkStop() on the robot
# Kept separate from agentGo so the start-trigger and safety-stop don't race.


@app.route("/agentStop/<id>", methods=["GET", "PUT"])
def agentStopReq(id):
    id = int(id)
    if request.method == "GET":
        if 0 < id and id <= NUM_ROBOTS:
            return jsonify(data["agentStop"][id-1])
    if request.method == "PUT":
        if 0 < id and id <= NUM_ROBOTS:
            data["agentStop"][id-1] = request.json
            return jsonify(data["agentStop"][id-1])

# obstacle route (ArUco marker 18)
# tracker2.py PUTs the current position here whenever marker 18 is visible.
# GET /obstacle returns {"id":18, "position":[x,y,theta], "found":bool}
