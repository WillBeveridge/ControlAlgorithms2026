...
cv2.imshow runs on the main thread (required on Windows).
"""
...
X_MIN, X_MAX    = -1.706,  1.706 
Y_MIN, Y_MAX    = -0.921,  0.921
OUTER_MARGIN    =  0.188
INNER_MARGIN    =  0.094
DRIVE_SPEED     =  0.18
MAX_W           =  6.0
...
def main():
    ...
    # Don't start the display thread inside tracker — we'll handle display here
    ...
    # Display loop runs on main thread (required on Windows)
