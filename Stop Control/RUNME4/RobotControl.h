...
public:
    ...
    // Returns true if the target was reached, false if stopped early by checkStop().
    bool moveTo(float X, float Y)
    {
        ...
        while (err > Err)
        {
            // ── Safety stop check every 200 ms ───────────────────────────────
            if ((unsigned long)(micros() - lastStopCheck) >= STOP_CHECK_INTERVAL_US)
            {
                lastStopCheck = micros();
                if (checkStop())
                {
                    drive(0, 0);
                    return false;   // stopped early — caller should not send more waypoints
                }
            }
    ...
    void getDriveCmd()
    {
        ...
        while (1)
        {
            if (Serial.available())
            {
                ...
                {
                    float v = req["v"].as<float>();
                    float w = req["w"].as<float>();
                    drive(v, w);
                    return;
                }
            }
